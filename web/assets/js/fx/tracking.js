...
(function ($) {
	...
	var getGa = function () {
		...
		if (!!window.dataLayer && !!window.dataLayer.push) {
			return function ga(gaAction, gaCat, cat, action, label, value, options, category, ecommerce) {
				if (gaCat === 'pageview') {
	...
	/* jshint maxparams:7 */
	var sendEvent = function (cat, action, label, value, options, category, ecommerce) {
		var ga = getGa();
		cat = cat || '';
		options = cat.options || options || { nonInteraction: 1 };
		if (!html.filter('[data-no-ga]').length) {
			ga('send', 'event', cat, action, label, value, options, category, ecommerce);
		} else {
