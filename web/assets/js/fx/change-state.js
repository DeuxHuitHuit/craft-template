...
(function ($, undefined) {
	...
	var doSetItemState = function (item, state, flag, flagPrefix) {
		if (!item || !item.length || !item[0]) {
			App.log('Called `doSetItemState` on an empty element collection, aborting.');
			return;
		}

		var flagClass = (!!flagPrefix ? flagPrefix : 'is-') + state;
	...
	var setItemState = function (item, state, flag, flagPrefix) {
		//Flag class
		...
		App.modules.notify('changeState.begin', {
			item: item,
			state: state,
			flag: flag,
		...
		//Execute change
		doSetItemState(item, state, flag, flagPrefix);
	...
	var processItem = function (item, state, action, callbacks, flagPrefix) {
		var flagClass = (!!flagPrefix ? flagPrefix : 'is-') + state;
		...
		if (action === 'toggle') {
			setItemState(item, state, !curBoolState, flagPrefix);
			...
			if (!curBoolState) {
				setItemState(item, state, true, flagPrefix);
			...
			if (curBoolState) {
				setItemState(item, state, false, flagPrefix);
		...
		if (data && data.item && data.state && data.action) {
			data.item.each(function () {
				...
				if (isMinWidth && isMaxWidth) {
					processItem(t, data.state, data.action, data.callbacks, data.prefix);
				}
			});
		}
	};
